...
export function PostLayoutPreview({
  ...
  const previewClassName = postType === "story" ? "relative mx-auto aspect-[9/16] w-full max-w-[320px]" : "relative aspect-square w-full";
  const [currentIndex, setCurrentIndex] = useState(0);
  const resolvedMediaItems = mediaItems.length > 0 ? mediaItems : imageUrl ? [{ imageUrl, imagePath: imageUrl }] : [];
      ...
      <div className="mt-4 overflow-hidden rounded-[28px] border border-slate-200 bg-white">
        {currentImageUrl ? (
          <div className={previewClassName}>
            <Image src={currentImageUrl} alt={dictionary.preview.imageAlt} fill className="object-cover" unoptimized />
          </div>
        ) : (
          <div
            className={`flex items-center justify-center bg-slate-100 text-sm text-slate-500 ${
              postType === "story" ? "mx-auto aspect-[9/16] w-full max-w-[320px]" : "aspect-square"
            }`}
          >
